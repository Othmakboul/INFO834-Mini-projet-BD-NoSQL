...
def ecouter_redis_pour_web():
    ...
    for message in pubsub.listen():
        if message['type'] == 'message':
            data = message['data']
            # Redis can return bytes if not decoded at source
...
def handle_message(data):
    ...
    if user and msg:
        ...
        if is_private and target:
            ...
            if target_sid:
                # Local helper to ensure sid is string
                sid_str = target_sid if isinstance(target_sid, str) else target_sid.decode('utf-8')
                emit('new_msg', payload, room=sid_str)
...
def handle_add_contact(data):
    ...
    if success:
        ...
        if target_sid:
            ...
            emit('conversations_data', target_convs, room=sid_str)
            emit('notification', {'success': True, 'message': f"{user} vous a ajouté à ses contacts !"}, room=sid_str)
...
def handle_get_stats():
    """Récupère les statistiques MongoDB pour le dashboard."""
    try:
        top_senders = mongo.get_top_senders()
        most_solicited = mongo.get_most_solicited()
        total_messages = mongo.collection.count_documents({})
        emit('stats_data', {
            'top_senders': top_senders,
            'most_solicited': most_solicited,
            'total_messages': total_messages
        }, room=request.sid)
    except Exception as e:
        emit('notification', {'success': False, 'message': f"Erreur stats: {e}"}, room=request.sid)

@socketio.on('search_messages')
def handle_search_messages(data):
    """Recherche des messages par mot-clé ou plage horaire."""
    keyword = data.get('keyword')
    time_range = data.get('time_range') # {'start': 'HH:MM', 'end': 'HH:MM'}
    
    try:
        if keyword:
            results = mongo.search_by_keyword(keyword)
        elif time_range:
            results = mongo.search_by_time_range(time_range['start'], time_range['end'])
        else:
            emit('notification', {'success': False, 'message': "Paramètre de recherche manquant."}, room=request.sid)
            return
            
        emit('search_messages_results', {'results': results}, room=request.sid)
    except Exception as e:
        emit('notification', {'success': False, 'message': f"Erreur recherche: {e}"}, room=request.sid)
...
def handle_kick_group_member(data):
    ...
    if success:
        ...
        if target_sid:
            sid_str = target_sid if isinstance(target_sid, str) else target_sid.decode('utf-8')
            socketio.server.leave_room(sid_str, group_name, namespace='/')
            convs = mongo.get_user_conversations(target)
            emit('conversations_data', convs, room=sid_str)
            emit('kicked_from_group', {'group_name': group_name}, room=sid_str)
            emit('notification', {'success': False, 'message': f"Vous avez été exclu du groupe {group_name}."}, room=sid_str)
